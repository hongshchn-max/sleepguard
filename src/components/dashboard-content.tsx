...
import { DashboardAnomalies } from '@/components/story/dashboard-anomalies';
...
export function DashboardContent() {
  ...
  if (!profile) {
    return <div className="flex h-[60vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-spectral border-t-transparent" /></div>;
  }

  return (
    <DashboardAnomalies>
    <div className="mx-auto max-w-lg space-y-6 p-4 pt-8">
      <h1 className="font-display text-2xl font-light">{t('greeting')}{profile.display_name ? `, ${profile.display_name}` : ''}</h1>

      {/* Countdown */}
      <div className={cn('dream-glass dream-glow rounded-2xl p-6 text-center', countdown.isPast ? 'border-somnia-rose/20' : 'border-spectral/20')}>
        <p className="mb-2 text-sm text-white/45">{countdown.isPast ? t('pastBedtime') : t('bedtimeIn')}</p>
        <div className="flex items-center justify-center gap-2 text-4xl font-bold tabular-nums">
          <span>{String(countdown.hours).padStart(2, '0')}</span>
          <span className="text-white/35">{t('hours')}</span>
          <span>{String(countdown.minutes).padStart(2, '0')}</span>
          <span className="text-white/35">{t('minutes')}</span>
          <span className="text-2xl">{String(countdown.seconds).padStart(2, '0')}</span>
          <span className="text-lg text-white/35">{t('seconds')}</span>
        </div>
        {countdown.isPast && (
          <Link href="/chat" className="mt-4 inline-block rounded-lg bg-spectral px-6 py-3 font-semibold text-void transition-all hover:bg-spectral-light hover:scale-105">
            {t('talkToLuna')}
          </Link>
        )}
      </div>
      ...
      <div className="dream-glass rounded-xl p-4">
        <div className="flex items-center gap-3">
          <span className="font-display text-3xl text-spectral">{'\u2726'}</span>
          <div>
            {profile.current_streak > 0
              ? <p className="text-lg font-bold">{profile.current_streak} {t('streak')}</p>
              : <p className="text-white/45">{t('streakEmpty')}</p>}
          </div>
        </div>
      </div>
      ...
      {/* Today's Status */}
      <div className="dream-glass rounded-xl p-4">
        <h3 className="mb-3 text-sm font-medium text-white/45">{t('todayStatus')}</h3>
        <div className="flex items-center justify-between">
          <span className={cn('rounded-full px-3 py-1 text-sm font-medium',
            todayLog === null ? 'bg-white/10 text-white/45' : todayLog.achieved ? 'bg-verdant-mist/20 text-verdant-mist' : 'bg-somnia-rose/20 text-somnia-rose')}>
            {todayLog === null ? t('statusPending') : todayLog.achieved ? t('statusAchieved') : t('statusMissed')}
          </span>
          <button onClick={() => setShowLogModal(true)} className="rounded-lg bg-spectral/20 px-4 py-2 text-sm text-spectral transition-colors hover:bg-spectral/30">
            {t('logSleep')}
          </button>
        </div>
      </div>
...
function SleepLogModal({ profile, onClose, onLogged }: { profile: Profile; onClose: () => void; onLogged: (log: TodayLog) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="dream-glass w-full max-w-sm space-y-4 rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="font-display text-lg font-light">{t('logSleep')}</h3>
        <div>
          <label className="mb-1 block text-sm text-white/45">{t('bedtimeLabel')}</label>
        ...
        <div>
          <label className="mb-1 block text-sm text-white/45">{t('waketimeLabel')}</label>
          <input type="time" value={waketime} onChange={(e) => setWaketime(e.target.value)}
            className="w-full rounded-lg border border-spectral/8 bg-void px-4 py-3 text-white focus:border-spectral-dim focus:outline-none" />
        </div>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-lg border border-spectral/8 py-3 text-white/60">{tc('cancel')}</button>
          <button onClick={handleLog} disabled={!bedtime || loading} className="flex-1 rounded-lg bg-spectral py-3 font-semibold text-void disabled:opacity-50">
            {loading ? '...' : tc('save')}
          </button>
        </div>
      </div>
